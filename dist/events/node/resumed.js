...
exports.default = new NodeEventHandler_1.NodeEventHandler({
    name: 'resumed',
    description: '...',
    async listener(node, payload, players) {
        // @ts-ignore
        const commands = this.getExtension(ForgeLink_1.ForgeLink, true).commands.player.get("resumed");
        if (!commands)
            return;
        ...
        for (const command of commands) {
            forgescript_1.Interpreter.run({
                ...
                environment: { node, payload, players }
            });
        }
    }
});
